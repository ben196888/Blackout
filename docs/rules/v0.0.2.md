...
This partial route schematic shows the critical routes, not every location or road:

```mermaid
flowchart LR
  VO[Village Office] ---|main bridge: Day 3 closure| SC[School]
  VO --- TE[Temple]
  TE ---|footbridge| CL[Clinic]
  CL --- SC
  VO --- FO[Forest Station]
  FO ---|upper road: Day 2 closure| CO[Co-op]
  FO --- QU[Quarry]
  QU ---|southern causeway| BA[Barn]
  BA --- CO
  BA --- FI[Field]
  FI --- SC
  SC --- CO
...
Neighborhood overview: each line represents one or more actual roads, not an additional movement shortcut.
