...
- A Mesh holder standing at a high-ground site gains a two-way radio link to specific distant nodes, without adding a walking road or covering the destination's whole neighborhood. The links are Lookout–Field on every map, Quarry–Dock on Town and Valley, and Observatory–Shelter on Valley. Both endpoints still require Mesh. Lookout remains a side trip, and Town's two high-ground sites remain concentrated in Ridge: the player must take the exposure and travel risk to provide those links. The links are separate from high-ground sight, so enclosed recipients can receive Mesh.
...
These are explicit new-scenario observation and radio changes and need rules-page and secrecy tests. A Radio Hut does not grant free communication merely by its name.
...
The draft deliberately changes map selection, graph topology, supplies, spawns, repeated professions, exposure classification, evacuation candidates, observation and radio coverage. It retains the v0.0.1 meal/starvation behavior and radio pricing, including the Day 5 radio exception.
