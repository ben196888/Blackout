...
| Ridge | Forest Station, Shrine, Quarry, Lookout | Mesh vantage, exposed approaches, limited local supplies |
...
| Addition | Locations | Connections and decisions |
| --- | --- | --- |
| Upland | Community Hall, Reservoir, Orchard, Radio Hut, Observatory | Connects Core, Ridge and Farm; food, batteries, Mesh spur |
...
- Remove passive high-ground sight in v0.0.2. Standing on a hill does not identify distant survivors or reveal their positions. Shrine no longer grants the legacy global sight ability. Co-located survivors still know who is there.
- A Walkie-talkie broadcast reaches all living holders in the sender's neighborhood and holders at nodes directly connected to the sender's node across a neighborhood boundary. Other nodes in the neighboring neighborhood are out of range. For example, Barn reaches all Farm nodes plus Field and Quarry on all three map scales. A closed road still marks radio proximity and does not block the broadcast. Co-located holders also hear it. Keep the current 40-character limit, battery cost, and requirement that recipients hold Walkie-talkie.
- A Reservist sender adds one road beyond that ordinary Walkie-talkie footprint, measured from any covered node. This is a signal-range bonus, so road closures do not shorten it. It does not change movement range or the recipient's method requirement.
- Mesh is an addressed, 40-character message between living Mesh holders. Its direct radio footprint matches the ordinary Walkie-talkie footprint: the sender's whole neighborhood plus nodes directly connected to the sender across a neighborhood boundary. Closed roads do not block radio. One living, Mesh-equipped player in that direct footprint can forward the packet once using their own direct footprint; the sender and recipient do not gain a second relay. Forwarding remains automatic and free, and the relay does not see the message. A potential relay destination is reachable only when that intermediary is actually present and equipped. Keep one battery per two sends, with the existing Day 5 surcharge.
- A Mesh holder standing at high ground can exchange messages with holders at open nodes in designated neighborhoods: Lookout covers Ridge and Farm, Quarry covers Ridge and Works on Town and Valley, and Observatory covers Upland and Core on Valley. These are two-way radio links, not passive sight or walking roads. They reveal no one unless a Mesh message is addressed and delivered. Enclosed nodes do not join this broad footprint, though ordinary local Mesh coverage still applies.
- Keep the narrow links to hard-to-reach nodes: Lookout–Field on every map, Quarry–Dock on Town and Valley, and Observatory–Shelter on Valley. These also work both ways with Mesh at both endpoints; the Shelter link intentionally reaches an enclosed node. No high-ground link gives coverage to an entire additional neighborhood. Lookout remains a side trip, and Town's two sites remain concentrated in Ridge: a player must take the travel and exposure risk to use them.
- The Student's unaided Mesh reach adds one road beyond the ordinary direct Mesh footprint, including any high-ground link from their current site. That bonus applies only to the Student's own transmission, not to a relay's retransmission. Road closures do not shorten radio range.
...
These are explicit new-scenario radio changes and need rules-page and secrecy tests. A Radio Hut does not grant free communication merely by its name.
...
The draft deliberately changes map selection, graph topology, supplies, spawns, repeated professions, exposure classification, evacuation candidates and radio coverage, and removes passive high-ground sight. It retains the v0.0.1 meal/starvation behavior and radio pricing, including the Day 5 radio exception.

A large-group absence policy is not yet selected. The current readiness barrier applies until a replacement is agreed. The issue tracker owns that decision and playtest acceptance criteria. This version must not be marked current until its implemented behavior and validation match the rulebook.
